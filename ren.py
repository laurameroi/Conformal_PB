...
class ContractiveREN(nn.Module):
    ...
    def __init__(
        ...
        if internal_state_init is None:
            self.register_buffer('x', torch.zeros(1, 1, dim_internal))
        else:
            self.register_buffer('x', internal_state_init.reshape(1, 1, dim_internal))
    ...
    def run(self, u_sequence):
        """
        Runs the ContractiveREN model over a sequence of inputs.

        Args:
            u_sequence (torch.Tensor): batch of input sequences of shape (batch_size, horizon, dim_in).

        Returns:
            y_sequence (torch.Tensor): batch of output sequences of shape (batch_size, horizon, dim_out).
        """
        # 1. Reset the internal state of the model
        self.reset()
        # Update the REN parameters ONCE before the trajectory starts.
        self._update_model_param()

        batch_size, horizon, dim_in = u_sequence.shape
        y_list = []

        # 2. Iterate over the time horizon
        for t in range(horizon):
            # Extract input at time t, keeping dimensions (batch_size, 1, dim_in)
            u_t = u_sequence[:, t:t + 1, :]

            # Forward pass updates the internal state 'model.x' automatically
            y_t = self.forward(u_t)

            y_list.append(y_t)

        # 3. Concatenate outputs along the time dimension
        y_sequence = torch.cat(y_list, dim=1)

        return y_sequence

    def __call__(self, u_sequence):
        """
        Args:
            u_sequence (torch.Tensor): batch of input sequences of shape (batch_size, horizon, dim_in).
